...
const EPSILON = 0.000000001; // tolerance
...
function intersect(
  ...
  const P1: point2D = edgeP.one.p;
  const P2: point2D = edgeP.two.p;
  const Q1: point2D = edgeQ.one.p;
  const Q2: point2D = edgeQ.two.p;

  const AP1 = A(P1, Q1, Q2);
  const AP2 = A(P2, Q1, Q2);
  ...
  if (Math.abs(AP1 - AP2) > EPSILON) {
    ...
    const AQ1 = A(Q1, P1, P2);
    const AQ2 = A(Q2, P1, P2);
    ...
    if (alpha_in_0_1 && beta_in_0_1) return IntersectionType.X_INTERSECTION;

    if (alpha_is_0 && beta_in_0_1) return IntersectionType.T_INTERSECTION_Q;

    if (beta_is_0 && alpha_in_0_1) return IntersectionType.T_INTERSECTION_P;

    if (alpha_is_0 && beta_is_0) return IntersectionType.V_INTERSECTION;
    ...
    if (alpha_in_0_1 && beta_in_0_1) return IntersectionType.X_OVERLAP;

    if (alpha_not_in_0_1 && beta_in_0_1) return IntersectionType.T_OVERLAP_Q;

    if (beta_not_in_0_1 && alpha_in_0_1) return IntersectionType.T_OVERLAP_P;

    if (alpha_is_0 && beta_is_0) return IntersectionType.V_OVERLAP;
  }
